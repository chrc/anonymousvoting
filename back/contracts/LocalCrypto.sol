...
contract LocalCrypto {
  ...
  function commitToVote(
      uint[4] memory params, uint[2] memory xG, uint[2] memory yG, uint[2] memory y,
      uint[2] memory a1, uint[2] memory b1, uint[2] memory a2, uint[2] memory b2) public view returns (bytes32) {
    return keccak256(abi.encodePacked(msg.sender, params, xG, yG, y, a1, b1, a2, b2));
  }
  ...
  // r = v - xz (mod p);
  // return(r,vG)
  function createZKP(uint x, uint v, uint[2] memory xG) public view returns (uint[4] memory res) {
      uint[2] memory _G;
      _G[0] = Gx;
      _G[1] = Gy;
      ...
      // Get g^{v}
      uint[3] memory vG = Secp256k1_noconflict._mul(v, _G);
  ...
  // a - b = c;
  function submod(uint a, uint b) public pure returns (uint) {
      uint a_nn;
  ...
  function verifyZKP(uint[2] memory xG, uint r, uint[3] memory vG) public view returns (bool) {
      uint[2] memory _G;
      _G[0] = Gx;
      _G[1] = Gy;
      ...
      // Get g^{r}, and g^{xc}
      uint[3] memory rG = Secp256k1_noconflict._mul(r, _G);
  ...
  // random 'w', 'r1', 'd1'
  function create1outof2ZKPNoVote(
      uint[2] memory xG, uint[2] memory yG, uint w,
      uint r2, uint d2, uint x) public view returns (uint[10] memory res, uint[4] memory res2) {
      ...
      bytes32 b_c = sha256(abi.encodePacked(msg.sender, xG, res));
  ...
  // random 'w', 'r1', 'd1'
  // TODO: Make constant
  function create1outof2ZKPYesVote(
      uint[2] memory xG, uint[2] memory yG, uint w,
      uint r1, uint d1, uint x) public view returns (uint[10] memory res, uint[4] memory res2) {
  ...
  function verify1outof2ZKP(
      uint[4] memory params, uint[2] memory xG, uint[2] memory yG, uint[2] memory y,
      uint[2] memory a1, uint[2] memory b1, uint[2] memory a2, uint[2] memory b2) public view returns (bool) {
    ...
    function createCommitment(uint r, uint b) public view returns (uint[2] memory) {
      ...
      // Sanity check that everything worked as expected.
                //Must be on the curve!
      require(Secp256k1_noconflict.isPubKey(c_affine));

      return c_affine;
    }

    // We need to re-create the commitment and check that it matches c.
    function openCommitment(uint[2] memory c, uint r, uint b) public view returns (bool) {
    ...
    // return t,n.
    function createEqualityProof(
        uint r1, uint r2, uint r3,
        uint[2] memory c1, uint[2] memory c2) public view returns (uint[2] memory t, uint n) {
      //Must be on the curve!
      require(Secp256k1_noconflict.isPubKey(c1));

      //Must be on the curve!
      require(Secp256k1_noconflict.isPubKey(c2));
      ...
      // TODO: add msg.sender
      uint h = uint(sha256(abi.encodePacked(msg.sender, G, Y, c1, c2, t)));
    ...
    function computeFirstHalfEquality(
        uint[2] memory c1, uint[2] memory c2,
        uint h, uint[2] memory t) public pure returns (uint[2] memory left) {
    ...
    // 2. Does nY == h*(c1-c2) + t
    function verifyEqualityProof(uint n, uint[2] memory c1, uint[2] memory c2, uint[2] memory t) public view returns (bool) {
      require(Secp256k1_noconflict.isPubKey(c1));
      require(Secp256k1_noconflict.isPubKey(c2));
      require(Secp256k1_noconflict.isPubKey(t));
    ...
    function createInequalityProof(
        uint b1, uint b2,
        uint r1, uint r2, uint r3, uint r4,
        uint[2] memory c1, uint[2] memory c2) public view returns (uint[2] memory t1, uint[2] memory t2, uint n1, uint n2) {
      ...
      // TODO: add msg.sender
      uint h = uint(sha256(abi.encodePacked(msg.sender, G, Y, c1, c2, t1, t2)));
    ...
    function computeSecondHalfInequality(
        uint[2] memory c1, uint[2] memory c2,
        uint[2] memory t2, uint h) public pure returns (uint[3] memory right) {
    ...
    function verifyInequalityProof(
        uint[2] memory c1, uint[2] memory c2,
        uint[2] memory t1, uint[2] memory t2, uint n1, uint n2) public view returns (bool) {

      require(Secp256k1_noconflict.isPubKey(c1));
      require(Secp256k1_noconflict.isPubKey(c2));
      require(Secp256k1_noconflict.isPubKey(t1));
      require(Secp256k1_noconflict.isPubKey(t2));
