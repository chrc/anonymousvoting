...
contract AnonymousVoting is Owned {
  ...
  // to complete the job...
  constructor(uint _gap, address payable _charity) public {
    G[0] = Gx;
    G[1] = Gy;
    state = State.SETUP;
  ...
  function setEligible(address[] memory addr) onlyOwner public {
  ...
  function beginSignUp(
      string memory _question, bool enableCommitmentPhase, uint _finishSignupPhase,
      uint _endSignupPhase, uint _endCommitmentPhase, uint _endVotingPhase,
      uint _endRefundPhase, uint _depositrequired) inState(State.SETUP) onlyOwner public payable returns (bool) {
   ...
   */
  function submitCommitment(bytes32 h) inState(State.COMMITMENT) public {
  ...
  function submitVote(
      uint[4] memory params, uint[2] memory y, uint[2] memory a1,
      uint[2] memory b1, uint[2] memory a2, uint[2] memory b2) inState(State.VOTE) public returns (bool) {
  ...
  function computeTally() inState(State.VOTE) onlyOwner public {
  ...
  function withdrawRefund() inState(State.FINISHED) public {
  ...
  function verify1outof2ZKP(uint[4] memory params, uint[2] memory y, uint[2] memory a1,
      uint[2] memory b1, uint[2] memory a2, uint[2] memory b2) public view returns (bool) {
